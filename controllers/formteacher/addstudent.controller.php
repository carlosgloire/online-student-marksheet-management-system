...
if (isset($_POST['add'])) {
    ...
    $paddress = htmlspecialchars($_POST['parentAdd']);
     ...
     $existing_regnumber_query = $db->prepare("SELECT * FROM students_per_class WHERE regnumber = :regnumber AND class_id = :class_id");
    $existing_regnumber_query->execute(array(':regnumber' => $regnumb, ':class_id' => $_SESSION['class_id']));
    ...
    if(empty($regnumb) || empty($fname) || empty($lname) || empty($pob) || empty($Dob) || empty($paddress)){
